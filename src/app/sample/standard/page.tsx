    ...
    <>
      ...
      <section className="mx-auto grid max-w-5xl gap-0 px-6 pt-16 pb-20 sm:grid-cols-[1fr_auto_1.1fr] sm:items-center sm:gap-6 sm:pt-24 sm:pb-28">
        <div className="sm:pr-4">
          <h1 className="font-serif text-4xl leading-[1.5] sm:text-5xl">
            あなたらしさを、
            <br />
            いちばん似合う形に。
          </h1>
          <p className="mt-7 max-w-sm text-sm leading-loose text-[#6b5d54]">
            一人ひとりの髪質・骨格に合わせたカウンセリングで、
            <br />
            なりたい自分に静かに寄り添うヘアサロンです。
          </p>
          <p className="mt-8 text-[11px] tracking-[0.15em] text-[#9c5a56]">
            完全予約制・完全個室 — 駅徒歩3分
          </p>
          <Link
            href="/sample/standard/access"
            className="mt-8 inline-block rounded-sm bg-[#9c5a56] px-9 py-3.5 text-xs tracking-[0.15em] text-white shadow-sm transition hover:bg-[#7c4844]"
          >
            ご予約はこちら
          </Link>
        </div>
        ...
        <div
          aria-hidden
          className="hidden py-10 text-[11px] tracking-[0.4em] text-[#9c5a56] sm:block"
        ...
        >
          <span
            aria-hidden
            className="absolute -left-3 top-8 text-[11px] tracking-[0.4em] text-[#9c5a56] sm:hidden"
            style={{ writingMode: "vertical-rl" }}
          >
            HAIR SALON
          </span>
      ...
      <section className="border-y border-[#e8d9d2] bg-[#faf8f4]">
        <div className="mx-auto grid max-w-3xl grid-cols-3 divide-x divide-[#e8d9d2]">
          {stats.map((stat) => (
            <div key={stat.label} className="px-4 py-8 text-center">
              <p className="font-serif text-2xl text-[#9c5a56] sm:text-3xl">
                {stat.value}
              </p>
              <p className="mt-1.5 text-[10px] tracking-wide text-[#6b5d54] sm:text-[11px]">
                {stat.label}
              </p>
            </div>
          ))}
      ...
      <section className="border-y border-[#e8d9d2] bg-white px-6 py-20 sm:py-24">
        <div className="mx-auto grid max-w-3xl gap-14 sm:grid-cols-[1.3fr_1fr]">
          <p className="font-serif text-2xl leading-[2] text-[#3a332c] sm:text-[1.7rem]">
            「今日、髪型変えた？」と聞かれるような、
            <br />
            さりげない変化を。
            <br />
          ...
          <ul className="space-y-7 border-l border-[#e8d9d2] pl-8">
            {reasons.map((reason) => (
              <li key={reason.name}>
                <p className="text-sm font-semibold text-[#3a332c]">
                  {reason.name}
                </p>
                <p className="mt-1.5 text-xs leading-relaxed text-[#6b5d54]">
                  {reason.description}
                </p>
              </li>
            ))}
